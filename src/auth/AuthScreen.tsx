...
const AuthScreen = () => {
  ...
  const handlePasswordReset = async () => {
    ...
    try {
      await sendPasswordResetEmail(auth, email);
      setInfoMessage('A password reset email has been sent to your email address.');
    } catch (error) {
      ...
      <div className='w-full max-w-md bg-white rounded-lg shadow-md p-8'>
        {infoMessage ? (
          <div className="text-center">
            <p className="text-green-500 mb-4">{infoMessage}</p>
            <button
          ...
          <>
            ...
            <p className='text-center text-sm text-gray-600 mt-4'>
              <a href='#' className='text-blue-600 hover:underline' onClick={handlePasswordReset}>
                Forgot Password?
              </a>
            </p>
          </>
